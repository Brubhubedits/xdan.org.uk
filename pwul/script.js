...
function startMainAnimations() {
  ...
  // Animate pricing tiers with stagger
  const pricingTiersAfter = document.querySelectorAll('.pricing-tier');
  pricingTiersAfter.forEach((tier, index) => {
    setTimeout(() => {
      tier.classList.add('animate');
    }, 1000 + (index * 200));
  });
...
function selectTier(tier) {
  ...
  // Show additional options based on tier
  document.querySelector('.additions-selector').classList.add('show');
  const subtitlesOption = document.querySelector('.addon-checkbox');
  if (tier === 'thumbnail') {
    subtitlesOption.style.display = 'none';
  } else {
    subtitlesOption.style.display = 'flex';
  }
...
function undoTierSelection() {
  ...
  // Reset subtitles option visibility
  document.querySelector('.addon-checkbox').style.display = 'flex';
...
function updatePayPalLink(total) {
  ...
  paypalButton.href = `https://paypal.me/pwulest/${total}`;
}
